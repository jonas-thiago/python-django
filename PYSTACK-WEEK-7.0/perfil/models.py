...
class Categoria(models.Model):
    ...
    valor_planejamentos = models.FloatField(default=0)
    ...
    def calcula_percentual_gasto_por_categoria(self):
        return int((self.total_gasto() * 100) / self.valor_planejamentos)
        #Adicione o try para evitar o ZeroDivisionError (Erro de divisão por zero)
        try:
            return (self.total_gasto() * 100) / self.valor_planejamentos
        except:
            return 0
...
class Conta(models.Model):
    ...
    icone = models.ImageField(upload_to="icnoes")
